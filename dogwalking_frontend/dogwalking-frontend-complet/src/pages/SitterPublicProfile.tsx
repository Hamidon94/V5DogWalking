...
const SitterPublicProfile = () => {
  ...
  const stats = [
    { label: "Avis", value: sitterInfo.reviewCount, icon: "⭐" },
    { label: "Services", value: sitterInfo.completedServices, icon: "✓" },
    { label: "Taux", value: "92%", icon: "📈" },
    { label: "Réponse", value: sitterInfo.responseTime, icon: "⏱️" },
  ];
          ...
          <CardContent className="pt-6">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-start gap-6">
                <div className="text-7xl">{sitterInfo.avatar}</div>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className="text-3xl font-bold">{sitterInfo.name}</h1>
                    {sitterInfo.verified && (
                      <Badge className="bg-blue-100 text-blue-800">✓ Vérifié</Badge>
                    )}
                    {sitterInfo.background_checked && (
                    ...
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-5 h-5 ${
                            i < Math.round(sitterInfo.rating)
                      ...
                      <span className="ml-2 font-semibold">
                        {sitterInfo.rating}/5 ({sitterInfo.reviewCount} avis)
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {sitterInfo.availability}
                    </div>
                    <div className="flex items-center gap-1">
                      <DollarSign className="w-4 h-4" />
                      À partir de {sitterInfo.pricePerHour}€/h
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-muted p-4 rounded-lg mb-6">
              <p className="text-sm">{sitterInfo.bio}</p>
            </div>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{sitterInfo.experience}</p>
              </div>

              <div>
                <h3 className="font-semibold mb-2">Services Proposés</h3>
                <div className="flex flex-wrap gap-2">
                  {sitterInfo.services.map((service, idx) => (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {sitterInfo.languages.map((lang, idx) => (
