    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-muted-foreground mb-6">
          ...
          <span>/</span>
          <span className="text-foreground">{promeneur.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="relative">
                    <Avatar className="w-32 h-32">
                      ...
                      <AvatarFallback className="text-2xl">{promeneur.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    {promeneur.verified && (
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col md:flex-row justify-between items-start mb-4">
                      <div>
                        <h1 className="text-3xl font-bold text-foreground mb-2">{promeneur.name}</h1>
                        <div className="flex items-center gap-4 text-muted-foreground mb-2">
                          <div className="flex items-center gap-1">
                            <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                            <span className="font-medium">{promeneur.rating}</span>
                            <span>({promeneur.reviewCount} avis)</span>
                          </div>
                          <div className="flex items-center gap-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Badge variant="secondary">
                            ...
                            Répond en {promeneur.responseTime}
                          </Badge>
                          <Badge variant="secondary">
                            Membre depuis {promeneur.joinDate}
                          </Badge>
                        </div>
                      </div>
            ...
            <Tabs defaultValue="about" className="space-y-6">
              ...
              <TabsContent value="about" className="space-y-6">
                <Card>
                  <CardHeader>
                    <CardTitle>À propos de {promeneur.name}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <p className="text-muted-foreground leading-relaxed">{promeneur.about}</p>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                      <div>
                        <h4 className="font-medium mb-2">Expérience</h4>
                        <p className="text-muted-foreground">{promeneur.experience}</p>
                      </div>
                      <div>
                        <h4 className="font-medium mb-2">Animaux possédés</h4>
                        <div className="flex gap-2">
                          {promeneur.pets_owned.map((pet) => (
              ...
              <TabsContent value="reviews" className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {promeneur.reviewCount} avis • Note moyenne: {promeneur.rating}/5
                    </CardDescription>
                  </CardHeader>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Réserver avec {promeneur.name}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((day) => (
                    <Badge 
                      key={day} 
                      variant={promeneur.availability.includes(day) ? "default" : "secondary"}
                      className={promeneur.availability.includes(day) ? "bg-dogwalking-green" : ""}
                    >
                      {day}
                    </Badge>
                  ))}
...
export default PromeneurProfile;